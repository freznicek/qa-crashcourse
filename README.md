...
(gdb) quit
```
As documentation claims invalid input should result in exit code of 2, which is not the case. Moreover we can see that application *aborted* due to an c++ exception `std::__throw_invalid_argument` leading to exit code of 134 (128+6).
